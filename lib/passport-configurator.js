...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var successRedirect = function(req, accessToken) {
    if (!!req && req.session && req.session.returnTo) {
      ...
      return appendAccessToken(returnTo, accessToken, req.query.state);
    }
    return appendAccessToken(options.successRedirect, accessToken, req.query.state) ||
      (link ? '/link/account' : '/auth/account');
  };

  var appendAccessToken = function(url, accessToken, state) {
    if (!accessToken) {
      return url;
    }
    return state ?
      url + '?access-token=' + accessToken.id + '&user-id=' + accessToken.userId + '&state=' + state:
      url + '?access-token=' + accessToken.id + '&user-id=' + accessToken.userId;
  };
        ...
        req.logIn(user, function(err) {
          ...
          if (info && info.accessToken) {
            if (!!options.json) {
              ...
              // if state data is available
              if (req.query.state) {
                res.cookie('state', req.query.state.toString(), {
                  signed: false,
                  maxAge: 1000 * info.accessToken.ttl,
                  domain: (options.domain) ? options.domain : null,
                });
